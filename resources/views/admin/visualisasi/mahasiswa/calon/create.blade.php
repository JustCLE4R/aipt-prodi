...
<section class="section-padding" style="margin-top: 9vh ;">
  <div class="section-header text-center">
    <h2 class="section-title wow fadeInDown" data-wow-delay="0.3s">Tambah Data Calon Mahasiswa</h2>
    <div class="shape wow fadeInDown" data-wow-delay="0.3s"></div>
  </div>
  <div class="container border rounded shadow" style="width:70%;">
    <form action="/admin/visualisasi/mahasiswa/calon" id="form" method="POST" enctype="multipart/form-data">
      <div class="row justify-content-between align-items-center p-3">
        ...
        <div class="col-lg-4 col-md-6 col-sm-12 my-2">
          ...
          <input  class="form-control @error('daya_tampung') is-invalid @enderror" type="number" name="daya_tampung" id="daya_tampung" value="{{ old('daya_tampung') }}" required>
          @if ($errors->has('daya_tampung'))
            <p class="error text-danger">{{ $errors->first('daya_tampung') }}</p>
          @endif
        </div>
        <div class="col-lg-4 col-md-6 col-sm-12 my-2">
          <label for="pendaftar" class="text-dark h6">Pendaftar</label>
          <input  class="form-control @error('pendaftar') is-invalid @enderror" type="number" name="pendaftar" id="pendaftar" value="{{ old('pendaftar') }}" required>
          @if ($errors->has('pendaftar'))
            <p class="error text-danger">{{ $errors->first('pendaftar') }}</p>
          @endif
        </div>
        <div class="col-lg-4 col-md-6 col-sm-12 my-2">
          <label for="lulus_seleksi" class="text-dark h6">Lulus Seleksi</label>
          <input  class="form-control @error('lulus_seleksi') is-invalid @enderror" type="number" name="lulus_seleksi" id="lulus_seleksi" value="{{ old('lulus_seleksi') }}" required>
